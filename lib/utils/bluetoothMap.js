	...
	gattCharacteristicsMapping: {
		...
		cycling_power_measurement: {
			...
			parseValue: value => {
				...
				//Watts with resolution of 1 TODO: units?
		...
		cycling_power_feature: {
			...
			parseValue: value => {
				...
				let flags = value.getUint16(0);
				let pedal_power_balance_supported = flags & 0x1;
				let accumulated_torque_supported = flags & 0x2;
				let wheel_revolution_data_supported = flags & 0x4;
				let crank_revolution_data_supported = flags & 0x8;
				let extreme_magnitudes_supported = flags & 0x10;
				let extreme_angle_supported = flags & 0x12;
				let top_and_bottom_dead_spot_angles_supported = flags & 0x14;
				let accumulated_energy_supported = flags & 0x16;
				let offset_compensation_indicator_supported = flags & 0x18;
				let offset_compensation_supported = flags & 0x20;
				let cycling_power_measurement_characteristic_content_masking_supported = flags & 0x22;
				let crankLengthAdjustmentSupported = flags & 0x24;
				let chainLengthAdjustmentSupported = flags & 0x26;
				let chainWeightAdjustmentSupported = flags & 0x28;
				let spanLengthAdjustmentSupported = flags & 0x30;
				let sensorMeasurementContext = flags & 0x32;
				let instantaneousMeasurementDirectionSupported = flags & 0x34;
				let factoryCalibrationDateSupported = flags & 0x36;
				let enhancedOffsetCompensationSupported = flags & 0x38;
				let distributeSystemSupport = flags & 0x40;
				let result = {};
				// TODO: finish this.
				if(wheel_revolution_data_supported) result.wheel_revolution_data_supported = wheelRevolutionDataSupported ? true : false;
				if(wheel_revolution_data_supported) result.wheel_revolution_data_supported = wheelRevolutionDataSupported ? true : false;
				if(wheelRevolutionDataSupported) result.wheel_revolution_data_supported = wheelRevolutionDataSupported ? true : false;
				if(wheelRevolutionDataSupported) result.wheel_revolution_data_supported = wheelRevolutionDataSupported ? true : false;
				if(wheelRevolutionDataSupported) result.wheel_revolution_data_supported = wheelRevolutionDataSupported ? true : false;
				if(wheelRevolutionDataSupported) result.wheel_revolution_data_supported = wheelRevolutionDataSupported ? true : false;
				if(wheelRevolutionDataSupported) result.wheel_revolution_data_supported = wheelRevolutionDataSupported ? true : false;
				...
				if(wheelRevolutionDataSupported) result.wheel_revolution_data_supported = wheelRevolutionDataSupported ? true : false;
				if(wheelRevolutionDataSupported) result.wheel_revolution_data_supported = wheelRevolutionDataSupported ? true : false;
				if(crankRevolutionDataSupported) result.crank_revolution_data_supported = crankRevolutionDataSupported ? true : false;
				if(multipleSensDataSupported) result.multiple_sensors_supported = multipleSensDataSupported ? true : false;
				return result;
			}
		},
	},
